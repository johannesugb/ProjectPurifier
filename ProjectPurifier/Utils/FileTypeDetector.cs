...
namespace ProjectPurifier.Utils
{
	...
	{
		public static bool MightBeBinary(string filename, int howManyConsecutiveZeroes = 3)
		{
			...
			try
			{
				...
				for (int i = 0; i < 512 && i < buffer.Length; i++)
				{
					...
					if (consecZeroesCnt >= howManyConsecutiveZeroes)
					{
						return true;
					}
				}
		...
		/// </returns>
		public static bool MightBeText(out Encoding encoding, string fileName, int windowSize = 512)
		{
